...
def inicializar_ag(funcion_evaluacion: Callable[[list[int]],Tuple[np.float64,None]],
    ...
    toolbox.register("select", tools.selTournament, tournsize=cantidad_torneo)
    return toolbox
...
def obtener_camino_ag(linea_cache_x:np.ndarray,
    ...
    if reanudar:
        try:
            ...
            raise(e(f"Ocurrio un error en la carga del chekcpoint: {e}"))
    ...
    generaciones_restantes = numero_generaciones - generacion_inicial
